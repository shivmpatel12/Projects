...
def group_by_centroid(restaurants, centroids):
    ...
    """
    locations = [[restaurant_location(x), x] for x in restaurants]
    closest = [[find_closest(x[0], centroids), x[1]] for x in locations]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    return key_of_max_value(dict([find_predictor(user, reviewed, x) for x in feature_fns]))
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            lst += [[res_name, user_rating(user, res_name)]]
        else:
            lst += [[res_name, predictor(r)]]
    return dict(lst)
